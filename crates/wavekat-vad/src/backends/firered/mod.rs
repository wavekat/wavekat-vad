...
impl FireRedVad {
    ...
    fn run_inference(&mut self, features: &[f32; N_MEL]) -> Result<f32, VadError> {
        // Create feature tensor: shape [1, 1, 80]
        let feat_array = ndarray::Array3::from_shape_vec((1, 1, N_MEL), features.to_vec())
            .map_err(|e| VadError::BackendError(format!("failed to create feature array: {e}")))?;

        let feat_tensor = Tensor::from_array(feat_array)
            .map_err(|e| VadError::BackendError(format!("failed to create feature tensor: {e}")))?;

        let cache_tensor = Tensor::from_array(self.caches.clone())
...
impl VoiceActivityDetector for FireRedVad {
    ...
    fn process(&mut self, samples: &[i16], sample_rate: u32) -> Result<f32, VadError> {
        ...
        if self.frame_count == 0 {
            ...
            let new_samples_f32 = &self.sample_buffer[..FRAME_SHIFT];
            // Convert back to i16 for extract_frame
            let new_samples_i16: Vec<i16> = new_samples_f32.iter().map(|&s| s as i16).collect();
            self.fbank
                .extract_frame(&new_samples_i16, &mut fbank_features);
...
mod tests {
    ...
    fn probabilities_match_python_reference() {
        ...
        for frame_idx in 0..num_frames {
            ...
            // Run inference
            let feat_array =
                ndarray::Array3::from_shape_vec((1, 1, 80), features.to_vec()).unwrap();
            let feat_tensor = Tensor::from_array(feat_array).unwrap();
            let cache_tensor = Tensor::from_array(caches.clone()).unwrap();
