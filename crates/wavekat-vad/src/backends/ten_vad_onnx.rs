...
//! entirely in Rust, without relying on the prebuilt C library. It produces identical
//! results to the FFI-based `ten_vad` backend.
//!
//! # Audio Requirements
//!
...
//! # Model
//!
//! Uses the same `ten-vad.onnx` model as the FFI backend:
...
/// The same model used by the FFI backend.
...
///
/// This backend produces the same results as the FFI-based `TenVad` backend
/// but without requiring the prebuilt C library.
///
/// # Example
///
/// ```no_run
...
///
/// let mut vad = TenVadOnnx::new().unwrap();
...
unsafe impl Send for TenVadOnnx {}

impl TenVadOnnx {
    /// Create a new TEN-VAD ONNX instance using the embedded model.
    pub fn new() -> Result<Self, VadError> {
        Self::from_memory(MODEL_BYTES)
    }

    /// Create a new TEN-VAD ONNX instance from model bytes in memory.
...
mod tests {
    ...
    fn create_ten_vad_onnx() {
        let vad = TenVadOnnx::new();
        assert!(vad.is_ok(), "Failed to create TenVadOnnx: {:?}", vad.err());
    }

    #[test]
    fn capabilities() {
        let vad = TenVadOnnx::new().unwrap();
...
/// Validation tests comparing FFI (ten-vad) vs pure ONNX (ten-vad-onnx) outputs.
/// These tests require both `ten-vad` and `ten-vad-onnx` features to be enabled.
#[cfg(test)]
#[cfg(all(feature = "ten-vad", feature = "ten-vad-onnx"))]
mod validation_tests {
    use super::*;
    use crate::backends::ten_vad::TenVad;

    /// Tolerance for probability comparison between FFI and ONNX backends.
    /// The preprocessing implementation may have small numerical differences
    /// (especially in pitch estimation), so we allow some tolerance.
    const TOLERANCE: f32 = 0.15;

    #[test]
    fn ffi_and_onnx_both_run() {
        // Just verify both can be created and run
        let mut ffi_vad = TenVad::new(256, 0.5).unwrap();
        let mut onnx_vad = TenVadOnnx::new().unwrap();

        let samples = vec![0i16; 256];

        let ffi_prob = ffi_vad.process(&samples, 16000).unwrap();
        let onnx_prob = onnx_vad.process(&samples, 16000).unwrap();

        println!("Silence: FFI={ffi_prob:.4}, ONNX={onnx_prob:.4}");

        // Both should produce valid probabilities
        assert!(ffi_prob >= 0.0 && ffi_prob <= 1.0);
        assert!(onnx_prob >= 0.0 && onnx_prob <= 1.0);
    }

    #[test]
    fn ffi_and_onnx_silence_comparison() {
        let mut ffi_vad = TenVad::new(256, 0.5).unwrap();
        let mut onnx_vad = TenVadOnnx::new().unwrap();
        ...
        let mut ffi_probs = Vec::new();
        let mut onnx_probs = Vec::new();

        for _ in 0..10 {
            let ffi_prob = ffi_vad.process(&silence, 16000).unwrap();
            let onnx_prob = onnx_vad.process(&silence, 16000).unwrap();
            ffi_probs.push(ffi_prob);
            onnx_probs.push(onnx_prob);
        }

        println!("Silence comparison (10 frames):");
        for (i, (ffi, onnx)) in ffi_probs.iter().zip(onnx_probs.iter()).enumerate() {
            let diff = (ffi - onnx).abs();
            println!("  Frame {i}: FFI={ffi:.4}, ONNX={onnx:.4}, diff={diff:.4}");
        }

        // Both should produce low probabilities for silence
        for &prob in &ffi_probs {
            assert!(prob < 0.5, "FFI should give low probability for silence");
        }
        for &prob in &onnx_probs {
            assert!(prob < 0.5, "ONNX should give low probability for silence");
        }
    }

    #[test]
    fn ffi_and_onnx_synthetic_speech_comparison() {
        let mut ffi_vad = TenVad::new(256, 0.5).unwrap();
        let mut onnx_vad = TenVadOnnx::new().unwrap();

        // Generate a synthetic speech-like signal (sine wave with harmonics)
        let mut samples = vec![0i16; 256];
        for i in 0..256 {
            let t = i as f32 / 16000.0;
            // Mix of frequencies typical in speech (fundamental + harmonics)
            let signal = (200.0 * 2.0 * std::f32::consts::PI * t).sin() * 0.5
                + (400.0 * 2.0 * std::f32::consts::PI * t).sin() * 0.25
                + (600.0 * 2.0 * std::f32::consts::PI * t).sin() * 0.125;
            samples[i] = (signal * 16000.0) as i16;
        }

        println!("Synthetic speech comparison:");
        for frame_idx in 0..5 {
            let ffi_prob = ffi_vad.process(&samples, 16000).unwrap();
            let onnx_prob = onnx_vad.process(&samples, 16000).unwrap();
            let diff = (ffi_prob - onnx_prob).abs();
            println!(
                "  Frame {frame_idx}: FFI={ffi_prob:.4}, ONNX={onnx_prob:.4}, diff={diff:.4}"
            );
        }
    }

    #[test]
    fn ffi_and_onnx_noise_comparison() {
        let mut ffi_vad = TenVad::new(256, 0.5).unwrap();
        let mut onnx_vad = TenVadOnnx::new().unwrap();

        // Generate random noise
        let samples: Vec<i16> = (0..256)
            .map(|i| ((i * 17 + 31) % 256) as i16 * 100 - 12800)
            .collect();

        println!("Noise comparison:");
        for frame_idx in 0..5 {
            let ffi_prob = ffi_vad.process(&samples, 16000).unwrap();
            let onnx_prob = onnx_vad.process(&samples, 16000).unwrap();
            let diff = (ffi_prob - onnx_prob).abs();
